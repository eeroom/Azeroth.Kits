...
namespace ExcelUserFunction
{
    ...
    public class RTD : Microsoft.Office.Interop.Excel.IRtdServer {
        ...
        public dynamic ConnectData(int cellId, ref Array parameters, ref bool getNewValues) {//每个单元格调用这个RTD的时候，都会调用一次这个方法
        ...
        public Array RefreshData(ref int cellCount) {
            ...
            int index = 0;
            foreach (var kv in dictCellAndParameter) {//第一行是单元格的ID，第二行是单元格的值，这是excel的约定，
